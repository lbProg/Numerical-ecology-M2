...
As a backup, we can perform a clustering of the stations each year. This will reveal which stations have similar communities and might reveal spatial structures.
...
```{r}
ggplot(data_clust, aes(x = longitude, y = latitude, color = factor(cluster))) +
  geom_point(size = 3) +
  facet_wrap(vars(year)) +
  theme_test()
```
...
To define ecological trajectories, we first need the entities (stations) and times (years).
...
station_cols <- data.frame(
  station = unique(data_env$station),
  col = pals::alphabet(18)
)
```
...
```{r}
pcoa <- trajectoryPCoA(trajs, traj.colors = station_cols$col, lwd = 2)
