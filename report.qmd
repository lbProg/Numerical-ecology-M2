...
Compute trajectory metrics for each replicate :
...
With only three dates, length = mean_speed and directionnality = - mean_angle.

```{r}
metrics_model <- metrics |>
  filter(metric %in% c("directionality", "length"))

ggplot(metrics_model, aes(x = longitude, y = value)) +
  geom_point() +
  facet_wrap(vars(metric), scales = "free") +
  theme_test()

ggplot(metrics_model, aes(x = latitude, y = value)) +
  geom_point() +
  facet_wrap(vars(metric), scales = "free") +
  theme_test()
```
...
```{r}
metrics_model_data <- pivot_wider(
  metrics,
  names_from = metric,
  values_from = value
)
...
There isn't a lot of variation in the trajectory metrics. All of them have relatively similar directionnality, length, mean angle and mean_speed.

As we saw with the trajectories before, the macrobenthic communities in the Florida Bay have experienced a shift in composition from 1996 and 2000, but this shift has been relatively similar for all stations.
