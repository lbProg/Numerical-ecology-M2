...
env_1997 <- data_env |> filter(year == "1997") |> select(c(3:6))
...
anova(rda_1996)
RsquareAdj(rda_1996)$adj.r.squared
```

```{r}
spe.good <- goodness(rda_1996)
sel.sp <- which(spe.good[, 2] >= 0.5)

triplot.rda(rda_1996, select.spe = sel.sp)
```