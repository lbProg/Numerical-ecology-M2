...
Can we consider the Eastern stations (with the lowest salinity) and the rest of the Bay as two different ecosystems ?
...
```{r}
anova(rda(data_abund_hell[1:18, ], data_env[c(3, 4)]))
```

There is a significant linear trend, so we have to detrend the data before db-MEM.

```{r}
data_abund_detrended <- resid(lm(as.matrix(data_abund_hell[1:18, ]) ~ ., data = data_env[c(3, 4)]))
```
...
We can also look at the unadjusted R² of each db-MEM variable (using function `scalog()` of the NEwR book) :
...
```{r}
RsquareAdj(dbmem_rda_2)$adj.r.squared
```
...
RDA1 and RDA2 are significant (for $\alpha = 0.05$). RDA3 sits close to the significance threshold. Its significance changes depending on runs of the anova. However, when running the anova with 9999 permutations (instead of the default 999), its p-value is close to 0.52. I will therefore consider it as non-significant.
...
The db-MEM analysis with three dm-MEMs explains 5.97% of the variance of the data.
The two represented spatial structures explain 5.97 * 0.79 = 4.71% of the total variance (not great).

```{r}
hell_2000 <- decostand(data_abund_pooled[37:54, -c(1, 2)], "hellinger")
quickmem <- quickMEM(data_abund_hell, as.matrix(data_env[c(3, 4)]))
```

```{r}
mem_dat <- cbind(MEM1 = quickmem$dbMEM_red_model$MEM1, stations_xy)
```

```{r}
ggplot(mem_dat, aes(x = longitude, y = latitude, size = abs(MEM1), color = sign(MEM1))) +
  geom_point()
```