...
metrics <- trajectoryMetrics(trajs) |>
  mutate(
    longitude = data_env$longitude[1:18],
    latitude = data_env$latitude[1:18]
  ) |>
  pivot_longer(
...
```{r}
ggplot(metrics, aes(x = longitude, y = latitude, color = value)) +
  geom_point() +
...
```{r}
#| fig-cap: Trajectory metrics computed from the ecological trajectories of the 18 Florida Bay stations

ggplot(metrics, aes(x = trajectory, y = value)) +
  geom_bar(stat = "identity") +
  facet_wrap(vars(metric), scales = "free") +
  labs(x = "Trajectory", y = "Metric value") +
  theme_test()
```
