...
The dataset used here can be found at <https://products.coastalscience.noaa.gov/nbi/data/project_data.aspx?project=124>.
...
-   Species abundances of benthic macrofauna
...
```{r}
#| fig-cap: Positions of all the stations sampled in 1996, 1997 and 2000
...
```{r}
#| fig-cap: Positions of the 18 stations that were sampled every year
...
```{r}
#| fig-cap: Map of the 18 stations sampled in 1996, 1997 and 2000
...
```{r}
#| fig-cap: Map of the 18 stations sampled each year. The depths measured in 2000 are displayed in color
...
```{r}
#| fig-cap: Salinities measured at each station in 1996, 1997 and 2000
...
ggplot(map_layer) +
  ...
  geom_point(
    data = data_env,
    aes(x = longitude, y = latitude, color = salinity),
    size = 2
  ) +
  geom_text_repel(
    data = data_env,
...
```{r}
#| fig-cap: Relationships between the four quantitative variables of the environmental dataframe
...
Depth show a negative correlation to longitude, but is not correlated to latitude. Salinity stays constant between the stations for each survey year, except for the three stations in the northeast that have lower salinities (as we saw before). Salinity also doesn't vary depending on depth.
...
plot_abundance <- ggplot(map_layer) +
  ...
  geom_point(
    data = data_env,
    aes(x = longitude, y = latitude, color = log(abundance)),
    size = 2
  ) +
  geom_text_repel(
    data = data_env,
  ...
  scale_color_viridis_c() +
  facet_wrap(vars(year), nrow = 3) +
  labs(color = "log(Abundance)") +
  theme_test()
```

```{r}
#| fig-cap: Maps of the specific richness and abundance (displayed in log scale) of the stations

ggarrange(plot_richness, plot_abundance)
```

Specific richness tends to be lower in the northeast stations. Total abundance doesn't really show any pattern.

## Aims of this study

As we saw, the stations in the Florida Bay present large salinity differences. This could strolngly affect the benthic community composition. We could expect to observe some spatial differences in the Bay.

This study will attempt to describe the spatiotemporal structure of the macrobenthic communities in the Florida Bay between 1996 and 2000. To do this, we will try to answer the following questions :
...
```{r}
#| code-fold: true
#| fig-cap: RDA of the Hellinger-transformed abundance data of the macrobenthic communities of the Florida Bay, constrained by the stations and the year as qualitative variables. Station and year centroids are represented by blue points.
...
The first two RDA axes represents 19% of the total variance of the data.

We can observe three groups of stations : a group of "low" stations in the bottom right quadrant, the northeast stations (24, 25, 29, 32 and 34) in the top right quadrant, and some "middle" stations on the left of the plot.

This could indicate that the northeast stations harbor different communities compared to the rest of the Florida Bay. For instance, these stations show a high relative abundance of the species *Phascolion strombi*, while the other stations show a low relative abundance for this species.

## Quantitative model

To further investigate the effect of space and time, I will consider them as quantitative variables. We can do this by taking the year value for time, and by using the latitude and longitude of the points for space. I will also use the depth variable.
...
forward.sel(
  data_abund_hell,
  data_env |> select(latitude, longitude, year_quant)
)
```
...
rda_quant <- rda(
  data_abund_hell ~ .,
  data = data_env |> select(latitude, longitude, year_quant)
)
anova(rda_quant)
```
...
```{r}
#| code-fold: true
#| fig-cap: RDA of the Hellinger-transformed abundance data of the macrobenthic communities of the Florida Bay, constrained by the stations coordinates and the year as quantitative variables.

ggplot() +
  geom_point(
    data = rda_coords$site_scores,
    aes(x = RDA1, y = RDA2, color = year)
  ) +
  geom_segment(
    data = rda_coords$species_scores,
    aes(
  ...
  geom_text_repel(
    ...
    aes(
      x = RDA1,
      y = RDA2,
      label = rep(data_env$station[1:18], 3)
    )
  ) +
  labs(
    x = paste0("RDA1 (", rda_coords$var_perc[1], "%)"),
...
The first two RDA axes account for 13% of the total variance of the data.

Here we can asses the impact of time on the communities. In 2000, the communities presented a highr relative abundance of *C. lyra* and *E. nebulosa*, as well as a lower relative abundance of *Paraeupolymnia sp. A* compared to 1996 and 1997. However, some species were not affected by time such as *S. pectinata* and *S. cornuta*, that were always at high relative abundance in the "low" stations (1, 5, 6, 7, 8, 11, 13, 14).
...
```{r}
#| fig-cap: Maps of the 7 db-MEM variables found by the dm-MEM analysis. Positive values are represented in blue and negative ones in red. The absolute value at each station is represented by the size of the associated point.

ggplot(data_MEM, aes(x = longitude, y = latitude, size = abs(value), color = factor(sign(value)))) +
  geom_point() +
  facet_wrap(vars(MEM)) +
  labs(size = "Value", color = "Sign") +
  theme_test()
```
...
```{r}
#| fig-cap: Dendrograms of the optimal clustering (average) of the stations for each sampling year.
...
```{r}
#| fig-cap: Silhouette values computed for every possible number of clusters on the station clusterings each year

ggplot(silhouettes, aes(x = k, y = sil)) +
  geom_bar(stat = "identity", width = 0.5) +
  facet_wrap(vars(year)) +
  labs(x = "Number of clusters", y = "Silhouette value") +
  theme_test()
```

According to the silhouette metric, the best number of clusters is always two. However, I would like a bit more precision so I will use the next best silhouette values (which aren't that much lower). Thus we have 8, 4 and 5 clusters for the years 1996, 1997 and 2000 respectively.
...
data_clust <- data_env |>
  ...
  mutate(
    cluster = c(
      unname(cutree(clust_96, 8)),
      unname(cutree(clust_97, 4)),
      unname(cutree(clust_00, 5))
    )
  )
```

```{r}
#| fig-cap: Maps of the stations each year, colored by their cluster. Clusters were found by cutting the clustering of the stations in the optimal number of clusters found with the silhouette metric. Cluster numbers are only valid within the same year.
...
The clusterings may reveal spatial structures (since stations close to eachother tend to be in the same clusters). For instance, stations \[29, 32, 34\], \[7, 8\], \[23, 26, 28\] and \[1, 5, 6, 14\] are always in the same cluster each year. The northeast stations (29, 32, 34) could therefore present different communities compared to the rest of the Florida Bay.

# Evolution of the communities through time

## Ecological trajectories

To define ecological trajectories, we need the entities (stations) and times (years). For the distance matrix, we will use the hellinger distance between the sites

```{r}
data_abund_hell <- decostand(data_abund_pooled[-c(1, 2, 3)], "hellinger")
...
```{r}
#| fig-cap: Ecological trajectories of the 18 stations of the Florida Bay between 1996 and 2000, represented on the two first PCoA axes
...
The first two PCoA axes account for 21% of the total variance of the data. Most trajectories seem to follow the same pattern : they move towards negative values of PCoA1.

However, some trajectories like those of stations 5, 11, 14, 23 and 26 show a move towards positive values of PCoA1 between 1997 and 2000.

We can also see that the trajectories of the northeast stations (24, 25, 29, 32, 34) are separated from the others on the PCoA space, but they follow the same general movement.

The macrobenthic communities in the Florida Bay have therefore experienced a shift in composition from 1996 to 2000, but this shift appears to have been relatively similar for all stations.

## Trajectory metrics

In order to check if all trajectories are similar, we can use trajectory metrics such as directionality, length, mean angle etc.

Here we will redefine our trajectories to include the replicates from the original data. This will allow us to test for linear relationships later.

```{r}
data_abund_hell <- decostand(data_abund[-c(1, 2, 3)], "hellinger")
dist_hell <- vegdist(data_abund_hell, "euclidean")
```
...
trajs <- defineTrajectories(
  dist_hell,
  sites = paste0(rep(data_env$station, each = 3), c("_1", "_2", "_3")),
  times = rep(as.numeric(data_env$year), each = 3)
)
```
...
metrics <- trajectoryMetrics(trajs) |>
  mutate(
    longitude = rep(data_env$longitude[1:18], each = 3),
    latitude = rep(data_env$latitude[1:18], each = 3)
  ) |>
  pivot_longer(
    length:internal_variance,
    names_to = "metric",
    values_to = "value"
  ) |>
  separate_wider_delim(trajectory, delim = "_", names = c("trajectory", "replicate")) |>
  mutate(
    trajectory = factor(trajectory, levels = order(as.numeric(trajectory)))
  )
```

With only three dates, length and mean speed are equal, and directionnality and mean angle have opposite variations. So we can simply use length and directionality.

We can check for linear trends :

```{r}
metrics_model <- metrics |>
  filter(metric %in% c("directionality", "length"))
  
metrics_model_data <- metrics_model |>
  pivot_wider(
    names_from = metric,
    values_from = value
  )

summary(lm(directionality ~ longitude, data = metrics_model_data))
summary(lm(length ~ longitude, data = metrics_model_data))
summary(lm(directionality ~ latitude, data = metrics_model_data))
summary(lm(length ~ latitude, data = metrics_model_data))
```

Trajectory length shows a significant positive linear correlation with both longitude and latitude. We can plot these results :

```{r}
metrics_model <- metrics_model |>
  filter(metric == "length")

lon_model_plot <- ggplot(metrics_model, aes(x = longitude, y = value)) +
  geom_point() +
  geom_abline(slope = 0.2614, intercept = 23.4171, color = "red") +
  annotate("text", x = -80.65, y = 1.75, label = paste("slope =", "0.26")) +
  labs(y = "Trajectory length") +
  theme_test()

lat_model_plot <- ggplot(metrics_model, aes(x = latitude, y = value)) +
  geom_point() +
  geom_abline(slope = 0.4175, intercept = -8.1664, color = "red") +
  annotate("text", x = 25.11, y = 1.75, label = paste("slope =", "0.42")) +
  labs(y = "Trajectory length") +
  theme_test()
```

```{r}
#| fig-cap: Significant linear relationships between the trajectory lengths of the 18 Florida Bay stations and their longitude/latitude

ggarrange(lon_model_plot, lat_model_plot)
```

The ecological trajectories between 1996 and 2000 show a linear trend with increasing values of trajectory length to the northeast. This means that the northeast stations have experienced a more important ecological shift than the stations in the southwest.

# Conclusion

To conclude, we have shown that space, time and the interaction between them all significantly affect the community composition of the macrobenthic fauna in the Florida Bay. Community composition differed along a northeast/southwest gradient, mainly so between the northeast inland stations, characterized by species such as *Phascolion strombi*, and the southwest, more offshore stations. This spatial structure could be linked to different environmental conditions such as salinity, which is lower in the northeast stations.

The communities have experienced an ecological shift between 1996, 1997 and 2000. Species such as *Cirrophorus lyra* and *Eupolymnia nebulosa* have become more abundant, although this shift did not distub the northeast/southwest structuring of the communities. Through the years, all stations have experienced a relatively similar ecological shift, but the amplitude of this shift has been more important in the northeast stations.
