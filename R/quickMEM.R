'quickMEM' <-
  function(Y,
    ...
    a <- system.time({
      ...
      if (is.null(myspat)) {
        ...
        if (!is.null(thresh)) {
          ...
          if (thresh < thresh.min + epsilon) {
            ...
            cat("\n The smallest value holding all observations together,",
                thresh.min,
                ",")
      ...
      if (nb.ev >= n - 1)
        stop(" Too many explanatory variables. They should be less than n - 1",
             call. = FALSE)
          ...
          if (method == "fwd") {
            ...
            if (ncol(Y) == 1) {
              ...
              if (norm.test[2] > 0.05) {
                ...
                Y.det <- scale(Y.det)
                fwd.sel <- forward.sel.par(Y.det,
                                           dbMEMbase,
                                           alpha = alpha,
                                           adjR2thresh = R2glob.a)
              }
              else {
                cat(
                  "\n ------------------------------------------------------------------"
      ...
      if (ncol(Y) == 1 || nb.sig.ev == 1) {
        ...
        nb.ax <-
          length(which(mod.axes.test[, ncol(mod.axes.test)] <= alpha))
      }
      # Compensation for internal loss of object by R if necessary
      ...
      if (method != "none") {
        if (nb.sig.ev == 1) {
          cat(" ",
              nb.sig.ev,
              "dbMEM eigenvector has been selected",
              "\n")
          if (nb.ax == 1) {
            cat("  The final model has",
                nb.ax,
                "significant canonical axis",
                "\n")
          }
          else{
            cat("  The final model has",
          ...
          cat(" ",
              nb.sig.ev,
              " dbMEM eigenvectors have been selected",
              "\n")
        }
        cat("  R2 of minimum (final) model = ", round(R2, 4), "                    ", "\n")
        cat("  Adjusted R2 of minimum (final) model = ",
            round(R2adj, 4),
            "        ",
            "\n")
        if (nb.ax == 1) {
    ...
    if (method == "none") {
      ...
      if (ncol(Y) > 1 && nb.sig.ev > 1) {
        ...
        table <-
          list(dbMEMbase, ev[1:nb.ev], fwd.sel, dbMEMred, mod, mod.test)
        names(table) <-
          c("dbMEM",
            "eigenvalues",
            "fwd.sel",
...
scatterutil.legend.bw.circle <- function (br0, sq0, sig0, clegend) {
  ...
  for (i in (2:(length(br0))))
    cha <- paste(cha, br0[i], sep = " ")
...
scatterutil.legend.circle.grey <-
