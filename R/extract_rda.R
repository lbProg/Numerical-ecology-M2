extract_rda <- function(res_rda, scaling = 1, select_spe = NULL, site_sc = "lc") {
  n_sp <- length(rda$colsum)
  ...
  BP_sc2 <- cbind(BP_sc2, matrix(0, nrow = nrow(BP_sc2), ncol = length(eig_val) - k))
  ...
  if (!is.null(res.rda$CCA$centroids)) {
    ...
    centroids_sc2 <- cbind(centroids_sc2, matrix(
      0,
      nrow = nrow(centroids_sc2),
      ncol = length(eig_val) - k
    ))
    colnames(centroids_sc2) <- colnames(F.sc2)
